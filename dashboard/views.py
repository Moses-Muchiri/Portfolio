...
def projects(request):
    ...
    context = {}
    profile = Information.objects.first() or {}
...
def projects_api(request):
    if request.method == 'POST':
        if request.POST.get('type') == 'create':
            ...
            if form.is_valid():
                form.save()
                return JsonResponse({'status': 'Project created successfully', 'code': 200})
            else:
                return JsonResponse({'status': 'Project creation failed', 'code': 400, 'errors': form.errors})

        elif request.POST.get('type') == 'update':
            try:
                id = int(request.POST.get('id'))
            except (TypeError, ValueError):
                return JsonResponse({'status': 'Invalid Project ID', 'code': 400})
            ...
            if request.POST.get('first', False):
                ...
                if project.exists():
                    ...
                    return JsonResponse({'status': 'Project Not Found', 'code': 404})
            ...
            if form.is_valid():
                form.save()
                return JsonResponse({'status': 'Project updated successfully', 'code': 200})
            else:
                return JsonResponse({'status': 'Project update Failed', 'code': 400, 'errors': form.errors})
